...
public class Test6
{
    ...
    private static void studyCompletable()
    {
        ...
        CompletableFuture<String> cf4 = cf2.thenApply(
                result2 ->
                {
                    // result2 为 CF2 的结果
                    // ...
        ...
        CompletableFuture<String> cf5 = cf1.thenCombine(cf2,
                (result1, result2) ->
                {
        ...
        /*
         * 异常处理: 在 CompletableFuture 的链条中, 如果某一步抛出了异常, 后续所有的正常操作都会被跳过, 直到遇到一个可以处理异常的方法
         * handle                  handleAsync                  两个参数 + 有返回值 + 没有异常也会执行 -> 它接收上一步的结果和异常, 经过处理后, 必须返回一个新的结果, 这意味着它可以 "吞掉" 异常并返回一个正常值, 或者把正常值转换成另一种格式
         * whenComplete            whenCompleteAsync            两个参数 + 无返回值 + 没有异常也会执行 -> 它接收上一步的结果 Result 和异常 Exception, 执行一些回调逻辑, 然后原封不动地把上一步的结果 (或异常) 传递给下一步
         * exceptionally           exceptionallyAsync           一个参数 + 有返回值 + 发生异常才会执行 -> 如果上一步发生异常则执行该方法, 它接收异常作为参数, 并返回一个新的结果, 这样可以让后续的操作继续进行下去
